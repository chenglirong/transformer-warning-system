"""阈值法异常检测 —— 基于国标 DL/T 722 / GB/T 7252 注意值。

最朴素的异常检测:任一特征气体浓度(或总烃)超过国标「注意值」即判为异常。
这是变压器在线监测最广泛使用的一线规则,不依赖训练,可解释性最强。

注意值来源(DL/T 722-2014《变压器油中溶解气体分析和判断导则》
附录 / GB/T 7252-2001 表 1,运行设备注意值):
...
CO/CO2 反映固体绝缘,作为辅助判据。本系统对外仍只输出 is_abnormal 二分类
(承系统边界 D-008:不暴露具体超标气体推出的故障类型)。
"""
...
# 运行设备注意值。键为「判定项」,非全部等于单一气体(总烃是合成项)
ATTENTION_VALUES: Dict[str, float] = {
    "h2": 150.0,
    "c2h2": 5.0,
    "total_hydrocarbon": 150.0,   # CH4 + C2H4 + C2H6 + C2H2
    "co": 300.0,
    "co2": 10000.0,
}
