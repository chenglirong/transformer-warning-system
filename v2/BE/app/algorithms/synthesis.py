...
答辩/论文口径(方案 B):正文可写「元数据约743条无时序,在分布约束下借助大模型辅助构建
360天演示时序」;工程落地是「分布保持的时序构造」(本模块),非大模型直接吐浓度。
与 Agent 报告用的 LLM(只写人话)分开说。合成数据上只论证方法可行,不吹真实精度。
真实故障样本各仅 2 条(Electrical/Thermal),故异常态锚点用其均值 + 固定 log-σ,如实交代样本稀疏。
