...
## 2026-06-18 三比值法改用 DL/T 722 表7 + CO/CO₂ 改比值法 + 打标改表3二分 + 全链路重跑(D-044)

### D-044:三比值法 IEC→DL/T 722-2014 表7 全栈对齐;CO/CO₂ 改 §10.2.3.1 比值判据;ground truth 打标改「表3二分+表7细分」;全数据重新合成+全模块重跑
...
- **真实新数字**(全部经磁盘读取或前台重跑亲核,**非中间过程回显**):
  - 检测三方法(vs 新 fault_state,n=360 异常91/25.3%):**阈值法(表3)** acc0.825/召回0.670/误报0.123/**F1 0.659**;**三比值法(表7)** acc0.583/**召回0.813**/误报0.494/F1 0.497;**iForest** acc0.803/召回0.604/误报0.130/F1 0.608。
