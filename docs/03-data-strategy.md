...
经查证,该公开集 **无正式论文、无标注方法说明、来源未标注**;原始 `Fault` 列 **97.6% 缺标**;故 ground truth 由国标 **DL/T 722-2014 自动打标**(非数据集自带权威标注):**表3 注意值判健康/异常二分、表7 三比值法细分故障类型**(D-044)。
**弥补**:① 国标打标 + 与原始 18 个有效标签交叉验证;② 量化数据可分性;③ 合成保持原始分布。
**数据可分性**(D-044 修正 D-040):采用表3 二分打标后,正常/异常各气体可分性 d 多 ≥ 0.43(C₂H₆ 达 0.96,仅 CO 0.14/H₂ 0.21 偏低),数据**具备合理可分性**;异常组总烃中位 179(略超注意值 150),但仍有 41% 异常样本未超注意值,属早期/轻微潜伏故障——这是 DGA 早期预警的行业难点,也是残余漏报/误报的主要来源。**修正说明**:早期(D-040)曾以 IEC 打标得出「弱可分(d 多<0.5)是统一根因」,改用国标表3 打标后该结论不再成立——原「弱可分」很大程度是旧打标把「绝对值不高、仅比值异常」样本也标为异常人为造成,现据实修正。
...
### 核心变更:用国标 DL/T 722-2014 做"自动打标"替代不可靠的 Fault 列

| 用途 | 原方案 | 新方案(D-044) |
|------|--------|--------|
| Ground truth 来源 | Fault 列(不可用) | DL/T 722-2014 对 743 行**自动打标**:表3 注意值定二分 + 表7 三比值法细分故障类型 |
| 异常检测对比的评估 | 阈值法/IF vs Fault 列 | 三方法(阈值/三比值/IF)vs 合成真值 fault_state(D-020) |
| 时序合成器的状态库 | 按 Fault 分组采样 | 按表3二分(健康)+表7故障类型分组采样 |
